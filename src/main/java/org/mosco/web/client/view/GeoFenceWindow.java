...
import org.mosco.web.client.model.EnumKeyProvider;
...
public class GeoFenceWindow implements Editor<GeoFence> {
    ...
    private GeoFence flush() {
        ...
        if (vertices != null) {
            org.mosco.web.shared.model.LonLat[] points = new org.mosco.web.shared.model.LonLat[vertices.length];
            for (int i = 0; i < vertices.length; i++) {
                vertices[i].transform(mapProjection, epsg4326);
                points[i] = new org.mosco.web.shared.model.LonLat(vertices[i].getX(), vertices[i].getY());
            }
