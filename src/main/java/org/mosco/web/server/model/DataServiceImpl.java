/*
 * Copyright 2013 Mosco (info@mosco.in)
 *
...
import static org.mosco.web.server.model.PasswordUtils.generateRandomUserSalt;
import static org.mosco.web.server.model.PasswordUtils.hash;
...
import org.mosco.web.shared.model.*;
...
public class DataServiceImpl extends RemoteServiceServlet implements DataService {
    ...
    @LogCall("Login '{0}'")
    @Override
    public User login(String login, String password, boolean passwordHashed) throws moscoException {
    ...
    @Transactional
    @Override
    public User login(String login, String password) throws moscoException {
    ...
    @RefreshBackendPermissions
    @Override
    public Device addDevice(Device device) throws moscoException {
    ...
    @ManagesDevices
    @Override
    public Device updateDevice(Device device) throws moscoException {
        if (device.getName() == null || device.getName().trim().isEmpty() ||
            device.getUniqueId() == null || device.getUniqueId().isEmpty()) {
            throw new ValidationException();
        }
    ...
    @RequireWrite
    @Override
    public GeoFence addGeoFence(GeoFence geoFence) throws moscoException {
    ...
    @RequireWrite
    @Override
    public GeoFence updateGeoFence(GeoFence updatedGeoFence) throws moscoException {
    ...
    public String sendCommand(Command command) throws AccessDeniedException {
        ...
        try {
            ...
            boolean moscoVersionBiggerThan_3_9 = false;

            Class<?> contextClass = Class.forName("org.mosco.Context");
            ...
            if (activeDevice == null) {
                ...
                Class<?> backendCommandClass = Class.forName("org.mosco.model.Command");
                Object backendCommand = backendCommandClass.newInstance();

                Class<?> backendJsonConverterClass = null;
                try {
                    backendJsonConverterClass = Class.forName("org.mosco.web.JsonConverter");
                } catch (ClassNotFoundException e) {
                    try {
                        backendJsonConverterClass = Class.forName("org.mosco.http.JsonConverter");
                    }
                    catch (ClassNotFoundException ee) {
                        moscoVersionBiggerThan_3_9 = true;
                    }
                } 

                Method objectFromJson;

                if (!moscoVersionBiggerThan_3_9)
                {
                    try {
                        Class<?> backendFactoryClass = Class.forName("org.mosco.model.Factory");
